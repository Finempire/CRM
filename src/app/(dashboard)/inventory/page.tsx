...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const stockItems = await prisma.stockItem.findMany({
    ...
    include: { vendor: { select: { name: true } }, _count: { select: { transactions: true } } },
    orderBy: { name: "asc" },
  });

  const categories = await prisma.stockItem.groupBy({ by: ["category"], _count: { id: true } });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="stat-card"><p className="text-xs text-muted-foreground font-medium">Total SKUs</p><p className="text-2xl font-bold mt-1">{stockItems.length}</p></div>
        <div className="stat-card"><p className="text-xs text-muted-foreground font-medium">Stock Value</p><p className="text-2xl font-bold mt-1">{formatCurrency(totalValue)}</p></div>
        <div className={`stat-card ${lowStock > 0 ? "border-yellow-200 bg-yellow-50/50" : ""}`}>
          <p className="text-xs text-muted-foreground font-medium flex items-center gap-1"><TrendingDown size={12} className="text-yellow-600" /> Low Stock</p>
        ...
        <div className={`stat-card ${outOfStock > 0 ? "border-red-200 bg-red-50/50" : ""}`}>
          <p className="text-xs text-muted-foreground font-medium flex items-center gap-1"><AlertTriangle size={12} className="text-red-500" /> Out of Stock</p>
          <p className={`text-2xl font-bold mt-1 ${outOfStock > 0 ? "text-red-600" : ""}`}>{outOfStock}</p>
        </div>
      </div>

      <div className="flex flex-wrap gap-3 items-center">
        <form className="flex items-center gap-2 flex-1" method="GET" action="/inventory">
          <div className="relative flex-1 max-w-xs">
            <Package className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={14} />
            <input type="text" name="q" defaultValue={query} placeholder="Search by name or code..."
          ...
          <button type="submit" className="px-4 py-2 bg-primary text-primary-foreground text-sm rounded-lg hover:bg-primary/90">Search</button>
        </form>
        <div className="flex flex-wrap gap-2">
          <a href="/inventory" className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${!category ? "bg-primary text-primary-foreground" : "bg-card border hover:bg-muted"}`}>All</a>
          {categories.map(c => (
            <a key={c.category} href={`/inventory?category=${c.category}`}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y">
              {stockItems.length === 0 ? (
                <tr><td colSpan={10} className="px-4 py-12 text-center text-muted-foreground"><Warehouse size={40} className="mx-auto mb-3 opacity-40" /><p>No inventory items found</p></td></tr>
              ) : (
                stockItems.map(item => {
                  ...
                  let statusLabel = "Normal", statusColor = "bg-green-100 text-green-700";
                  ...
                  return (
                    <tr key={item.id} className={`hover:bg-muted/30 transition-colors ${isOutOfStock ? "bg-red-50/20" : isLow ? "bg-yellow-50/20" : ""}`}>
                      ...
                      <td className="px-4 py-3"><p className="font-medium">{item.name}</p>{item.description && <p className="text-xs text-muted-foreground truncate max-w-[180px]">{item.description}</p>}</td>
                      <td className="px-4 py-3 text-xs"><span className="px-2 py-0.5 rounded bg-muted font-medium">{item.category}</span></td>
                      <td className="px-4 py-3 text-xs text-muted-foreground">{item.location || "—"}</td>
                      <td className="px-4 py-3 text-xs">{item.vendor?.name || "—"}</td>
                      <td className="px-4 py-3 text-right"><span className={`font-bold ${isOutOfStock ? "text-red-600" : isLow ? "text-yellow-600" : ""}`}>{currentStock.toLocaleString()} {item.unit}</span></td>
                      <td className="px-4 py-3 text-right text-muted-foreground text-xs">{minStock ? `${minStock.toLocaleString()} ${item.unit}` : "—"}</td>
                      ...
                      <td className="px-4 py-3"><span className={`status-chip ${statusColor}`}>{statusLabel}</span></td>
                    </tr>
                  );
                })
              )}
