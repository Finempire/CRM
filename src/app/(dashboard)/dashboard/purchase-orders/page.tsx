    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="stat-card">
          ...
          <p className="text-2xl font-bold mt-1">{formatCurrency(totalPOValue)}</p>
        </div>
        <div className="stat-card">
          <p className="text-xs text-muted-foreground font-medium">Pending Delivery</p>
          <p className="text-2xl font-bold mt-1 text-orange-600">{pending}</p>
        </div>
      </div>
              ...
              {purchaseOrders.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-4 py-12 text-center text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">POs are created from Order detail → Purchase Orders tab</p>
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={po.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/orders/${po.orderId}`} className="font-medium text-primary hover:underline flex items-center gap-1 text-xs">
                          {po.order.orderNumber} <ExternalLink size={11} />
                        </Link>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-medium text-xs">{po.vendor?.name || "—"}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        {po.vendor?.type && (
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs px-2 py-0.5 rounded bg-muted">{po.lines.length} items</span>
                      </td>
