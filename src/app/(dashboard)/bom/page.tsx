...
export default async function BomPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  const session = await auth();
  if (!session?.user) redirect("/login");

  const sp = await searchParams;

  // Get active orders for selector
  ...
  // If orderId is selected, fetch its BOM items
  ...
  const totalEstimatedCost = selectedOrder?.bomItems.reduce((s: number, i: any) => s + (i.estimatedAmount ? Number(i.estimatedAmount) : 0), 0) || 0;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-card rounded-xl border p-4 flex items-center gap-4">
        ...
        <form className="flex-1 flex items-center gap-3" method="GET" action="/bom">
          <select
            name="orderId"
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="stat-card">
              ...
              <p className="text-2xl font-bold mt-1">{selectedOrder.orderLines.reduce((s: number, l: any) => s + l.quantity, 0).toLocaleString()}</p>
            </div>
          </div>
