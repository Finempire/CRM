...
export const metadata: Metadata = { title: "TNA Calendar" };
...
export default async function TnaPage() {
  ...
  const orders = await prisma.order.findMany({
    ...
    include: { buyer: true, tnaMilestones: { orderBy: { plannedDate: "asc" } } },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-4">
        {orders.map(order => {
          const daysToShip = order.shipmentDate
            ? Math.ceil((new Date(order.shipmentDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
            : null;
        ...
        {orders.length === 0 && (
          <div className="bg-card rounded-xl border p-12 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">Create TNA milestones from the Order detail page → TNA tab</p>
          </div>
        )}
      </div>
        ...
        <div className="bg-card rounded-xl border overflow-hidden">
          <div className="px-6 py-4 border-b">
            <h2 className="font-semibold flex items-center gap-2 text-amber-600">
              <AlertTriangle size={18} /> Orders Missing TNA Plan ({ordersWithoutTna.length})
            </h2>
          </div>
