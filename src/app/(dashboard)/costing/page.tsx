  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-card rounded-xl border p-4">
        <form className="flex items-center gap-4 mb-4" method="GET" action="/costing">
          <div className="relative flex-1 max-w-sm">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
            <input type="text" name="q" defaultValue={query} placeholder="Search by Order # or Buyer..."
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-medium">
                        {hasCosting ? formatCurrency(Number(order.costing?.totalCost)) : "—"}
                      </td>
                      <td className="px-4 py-3 text-right font-medium text-blue-600">
                        {hasCosting ? formatCurrency(Number(order.costing?.totalRevenue)) : "—"}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {hasCosting ? (
                          <span className="status-chip bg-gray-100">{order.costing?.approvalStatus}</span>
                        ) : (
                          <span className="text-xs text-muted-foreground italic">Not created</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <Link href={`/costing/${order.costing?.id ?? order.id}`}
                          ...
                          title={hasCosting ? "View/Edit Costing" : "Create Costing"}>
